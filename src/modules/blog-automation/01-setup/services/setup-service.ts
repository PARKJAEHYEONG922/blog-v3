...
class SetupServiceClass {
    ...
    try {
      ...
      if (loadedSeoGuides && loadedSeoGuides.length > 0) {
        ...
        if (initialData?.seoGuidePath) {
          ...
          // localStorage에서 마지막 선택 ID 복원
          const savedSeoId = StorageService.getSelectedSeoGuideId();
          if (savedSeoId) {
            selectedSeoGuide = seoGuides.find((doc: SavedDocument) =>
              doc.id === savedSeoId
            ) || null;
          ...
          // 저장된 ID가 없거나 해당 문서를 못찾으면 기본 SEO 선택
          if (!selectedSeoGuide) {
            selectedSeoGuide = seoGuides.find((doc: SavedDocument) =>
  ...
  async crawlBlogContent(url: string): Promise<{ title: string; content: string } | null> {
    try {
      ...
      const { BlogCrawler } = await import('@/modules/blog-automation/01-setup/services/blog-crawler');
